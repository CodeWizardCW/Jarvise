...
fridayComs.push("close this tab");
...
function weather(location) {
  ...
  xhr.onload = function () {
    if (this.status === 200) {
      ...
      weatherCont[0].textContent = `Your Country: ${data.sys.country}`;
      weatherCont[1].textContent = `Location : ${data.name}`;
      weatherCont[2].textContent = `Today Temperature : ${ktc(data.main.temp)}`;
      weatherCont[3].textContent = `Today Weather type : ${data.weather[0].main}`;
...
recognition.onresult = function (event) {
  ...
  if (transcript.includes("tell about varun")) {
    readOut("varun is a donkey");
  }
  ...
  if (transcript.includes("connection status")) {
    readOut(`i am are ${connectivity} sir`);
  }
  // jarvis commands
  ...
  if (transcript.includes("Tell about yourself")) {
    readOut(
      "sir, i am a jarvis, a voice asistant made for browsers using javascript. I can do anything which can be done from a browser."
    );
  }
  ...
  if (transcript.includes("close this")) {
    ...
    document.querySelector(".commands").style.display = "Close Displaying Tab";
    if(window.innerWidth >= 401 ){
      window.resizeTo(250,250)
    }
    setup.style.display = "Close Displaying Tab";
  }
...
// classroom
  if (transcript.includes("open classroom")) {
    readOut("opening classroom");
    let a = window.open("https://classroom.google.com/");
    windowsB.push(a)
  }
// spotify
  if (transcript.includes("open map")) {
    readOut("opening maps");
    let a = window.open("https://www.google.co.in/");
    windowsB.push(a)
  }

  if (transcript.includes("open school website")) {
    readOut("opening school website");
    let a = window.open("https://www.apsnasirabad.in");
    windowsB.push(a)
  }

  if (transcript.includes("open translate") || transcript.includes("open camera")) {
